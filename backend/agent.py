...
- Check open times with check_availability and book with book_appointment.
...
- Before booking, you need a name, a callback number, and an agreed time. Ask for whatever \
is missing. Read the details back once before you book, and confirm again after.
- Save a note the moment something is worth passing on. Do not wait for the end of the call.
- If a caller describes a medical, legal or safety emergency, tell them to hang up and call \
emergency services immediately.

# Opening hours
Open {settings.open_days_label}, {settings.hours_label}. \
Default appointment length is {settings.slot_minutes} minutes.{knowledge}"""
...
async def summarise(transcript: list[dict[str, str]]) -> dict[str, Any]:
    ...
    instructions = (
        ...
        "Only list a follow-up if a person actually has to do something; an appointment "
        "already booked during the call is not a follow-up."
    )
    messages = [
